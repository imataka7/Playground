...
export const initEscrow = async (
    ...
    //@ts-expect-error
    const XTokenMintAccountPubkey = new PublicKey((await connection.getParsedAccountInfo(initializerXTokenAccountPubkey, 'finalized')).value!.data.parsed.info.mint);
    ...
    const createTempTokenAccountIx = SystemProgram.createAccount({
        ...
        lamports: await connection.getMinimumBalanceForRentExemption(AccountLayout.span, 'finalized'),
    ...
    const createEscrowAccountIx = SystemProgram.createAccount({
        ...
        lamports: await connection.getMinimumBalanceForRentExemption(ESCROW_ACCOUNT_DATA_LAYOUT.span, 'finalized'),
        fromPubkey: initializerAccount.publicKey,
    ...
    await connection.sendTransaction(tx, [initializerAccount, tempTokenAccount, escrowAccount], { skipPreflight: false, preflightCommitment: 'finalized' });
